...
const HeroSection = () => {
  ...
  const userImages = [child1, child2, child3, child4]; 
        ...
        >
          ...
          <div className="flex -space-x-3">
            {userImages.map((imgSrc, index) => (
              <div 
                key={index} 
                className="relative w-10 h-10 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center overflow-hidden"
              >
                <Image 
                  src={imgSrc} 
                  alt={`Student ${index + 1}`} 
